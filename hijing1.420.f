c     version 1.42
c     PP(I,11-15), PT(I,11-15) was not initialized before. This version corrects this.
c
c     version 1.41
...
!     Niseem Start here
                   IF (N_ST .LT. 9000) THEN ! Niseem                      
                      
!     WRITE(*,*) 'Niseem Test in->ELSE1-------',K(N_ST,2)
                      IF(K(N_ST,2).LT.91.OR.K(N_ST,2).GT.93) GO TO  381
!     WRITE(*,*) 'Niseem Test in->ELSE2-------',K(N_ST,2)
                      IDSTR=K(N_ST,2)
                      N_ST=N_ST+1
                   ENDIF        !Niseem
		ENDIF
!     Niseem end here
!WRITE(*,*) 'Test in->',N_ST,' ',IDSTR,' ',K(N_ST,2)
